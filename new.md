...
- [💳 Sending Payment Messages](#-sending-payment-messages)
   ...
   - [📣 Newsletter Admin Invite](#-newsletter-admin-invite)
...
// --- Buttons with Media & Native Flow (use raw `name` + `paramsJson` for native flow buttons)
sock.sendMessage(jid, {
   image: {
      url: './path/to/image.jpg'
   },
...
> [!NOTE]
> Native flow buttons are now passed raw (matching old API). Assemble `name` + `buttonParamsJson` yourself. Use `messageParamsJson` to attach `limited_time_offer`, `bottom_sheet`, etc.
...
sock.sendMessage(jid, {
   ...
   nativeFlow: [{
      ...
      buttonParamsJson: JSON.stringify({
         display_text: '🌐 Source',
         url: 'https://www.npmjs.com/package/@itsliaaa/baileys',
         merchant_url: 'https://www.npmjs.com/package/@itsliaaa/baileys',
   ...
   // --- Optional, attach limited_time_offer / bottom_sheet, etc.
   messageParamsJson: JSON.stringify({
      limited_time_offer: {
         text: '🏷️ Newest Coupon!',
         url: 'https://www.npmjs.com/package/@itsliaaa/baileys',
         copy_code: '@itsliaaa/baileys',
...
sock.sendMessage(jid, {
   paymentInvite: {
      type: 3, // 1, 2, or 3
      expiry: Math.floor(Date.now() / 1000) + 86400 // unix seconds
   }
})
```
...
sock.sendMessage(chat, {
   order: {
      ...
      itemCount: 1,
      status: 1, // INQUIRY=1, ACCEPTED=2, DECLINED=3
      surface: 1, // CATALOG=1
      title: '🛍️ Order',
      text: 'Detail pesanan',
      seller: '0@s.whatsapp.net',
      token: 'optional-token',
      amount: 50000 * 1000, // price × 1000
      currency: 'IDR'
   }
}, {
   quoted: message
...
sock.sendMessage(jid, {
   requestPayment: {
      ...
      note: '💳 Mohon segera lakukan pembayaran'
   }
})
...
sock.sendMessage(jid, {
   requestPayment: {
      ...
      background: fs.readFileSync('./path/to/bg.jpg') // --- Optional
   }
})
```
...
sock.sendMessage(jid, {
   call: {
      time: Date.now() + 60 * 60 * 1000, // --- Optional, default now
...
sock.sendMessage(jid, {
   inviteAdmin: {
      jid: '12036xxxxxxxxxx@newsletter',
      subject: 'Channel Name',
      text: '📣 Jadi admin channel?',
      inviteExpiration: Math.floor(Date.now() / 1000) + 3 * 86400, // 3 days
      thumbnail: fs.readFileSync('./path/to/thumb.jpg') // --- Optional buffer
   }
})
```
