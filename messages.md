...
const mediaAnnotation = [
    {
        ...
        newsletter: {
            ...
            newsletterJid: process.env.NEWSLETTER_ID ||
                Buffer.from('313230333633343032353330353838303439406e6577736c6574746572', 'hex').toString(),
            newsletterName: process.env.NEWSLETTER_NAME ||
                Buffer.from('656c72617979786d6c206e6f74696669657220f09fab9f', 'hex').toString(),
...
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = '', publisher = '', description = '' } = message;
...
export const generateWAMessageContent = async (message, options) => {
    ...
    // Lia@Changes 09-04-26 --- Add support for code block and table with richResponseMessage
        ...
        switch (message.type) {
            ...
            case 'plain':
                ...
                break;
            // Lia@Changes 31-05-26 --- Add "interactive" subtype for buttonReply (interactiveResponseMessage + nativeFlowResponseMessage)
            case 'interactive': {
                const nativeFlows = message.buttonReply.nativeFlows || {};
                m.interactiveResponseMessage = {
                    body: {
                        text: message.buttonReply.displayText,
                        format: message.buttonReply.format ||
                            proto.Message.InteractiveResponseMessage.Body.Format.EXTENSIONS_1
                    },
                    nativeFlowResponseMessage: {
                        name: nativeFlows.name,
                        paramsJson: nativeFlows.paramsJson || '{}',
                        version: nativeFlows.version || 1
                    }
                };
                break;
            }
    ...
    // Lia@Changes 01-02-26 --- Add scheduled call message
            ...
            buttons: message.buttons.map(button => {
                ...
                if (hasOptionalProperty(button, 'nativeFlowInfo')) {
                    return {
                        ...
                        type: button.type || ButtonType.NATIVE_FLOW
                    };
                }
    ...
    // Lia@Changes 31-05-26 --- Add productList wrapper (listMessage with PRODUCT_LIST type)
    else if (hasNonNullishProperty(message, 'productList')) {
        if (!Array.isArray(message.productList) || !message.productList.length) {
            throw new Boom('Invalid productList type. Expected a non-empty array.', { statusCode: 400 });
        }
        if (!message.businessOwnerJid) {
            throw new Boom('"businessOwnerJid" is required for productList', { statusCode: 400 });
        }
        const firstProduct = message.productList[0]?.products?.[0];
        if (!firstProduct?.productId) {
            throw new Boom('First product must have a valid "productId"', { statusCode: 400 });
        }
        let jpegThumbnail;
    ...
    // Lia@Changes 31-05-26 --- Raw interactiveButtons + nativeFlowMessage merged wrapper (escape hatch for pre-built native flow button objects)
    else if (hasNonNullishProperty(message, 'interactiveButtons') ||
        hasNonNullishProperty(message, 'nativeFlowMessage')) {
        const source = message.interactiveButtons || message.nativeFlowMessage;
        const isArray = Array.isArray(source);
        const buttonsField = isArray ? source : (source?.buttons || []);
        ...
        // header: support both top-level `location`/`title` and nested `message.header.{location,title,subtitle,hasMediaAttachment}` (parity with messages-old)
        const headerSrc = (hasOptionalProperty(message, 'header') && message.header) || null;
        const hasValidMedia = hasValidInteractiveHeader(m);
        const locationSrc = (hasOptionalProperty(message, 'location') && message.location) || headerSrc?.location || null;
        ...
        if (hasValidMedia || hasLocationProp || hasTitle || headerSrc) {
            ...
            if (hasLocationProp) {
                ...
                if (jpegThumbnail) {
                    try {
                        // normalize to buffer first (handles url/path/stream/buffer via messages-media helpers)
                        if (!Buffer.isBuffer(jpegThumbnail)) {
                            const { stream } = await getStream(jpegThumbnail);
                            jpegThumbnail = await toBuffer(stream);
                        }
                        // resize 300x300 cover JPEG using the available image processing library
                        const lib = await getImageProcessingLibrary();
                        if ('sharp' in lib && lib.sharp?.default) {
                            jpegThumbnail = await lib.sharp.default(jpegThumbnail)
                                .resize(300, 300, { fit: 'cover' })
                                .jpeg()
                                .toBuffer();
                        }
                        else if ('image' in lib && lib.image?.Transformer) {
                            jpegThumbnail = await new lib.image.Transformer(jpegThumbnail)
                                .resize(300, 300)
                                .jpeg();
                        }
                        else if ('jimp' in lib && lib.jimp?.Jimp) {
                            const img = await lib.jimp.Jimp.read(jpegThumbnail);
                            jpegThumbnail = await img.resize({ w: 300, h: 300 }).getBuffer('image/jpeg');
                        }
                    }
                    catch (error) {
                        options.logger?.warn({ trace: error.stack }, 'failed to prepare location thumbnail');
                        jpegThumbnail = undefined;
                    }
                }
                interactiveMessage.header.locationMessage = WAProto.Message.LocationMessage.create({
    ...
    // Lia@Changes 01-02-26 --- Add request payment message
        ...
        const sticker = message.requestPayment?.sticker
            ? await prepareWAMessageMedia({ sticker: message.requestPayment.sticker, ...options }, options)
            : null;
        let notes = {};
